...
hall::hall(int x, int y, int screen_Wj, int screen_Hj) : interfaceComponent() {
	...
	//------------------------------------------------------
	Thing::sizeX_Hall = static_cast<float>(numero_x);
	Thing::sizeY_Hall = static_cast<float>(numero_y);
	Thing::hall_x0 = x0;
	Thing::hall_y0 = y0;
...
void hall::setThingsInf(){
	Thing::size_global = static_cast<float>(size);
	Thing::x0_global = static_cast<float>(bloco_x0);
